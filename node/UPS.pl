...
if ($trapParams->{Enterprise}=~/MG-SNMP-UPS-MIB::upsmgTraps/) {
    if($trapParams->{TrapNumber}==17) {
        $cmd='shutdown -h +13';
    } elsif ($trapParams->{TrapNumber}==9||$trapParams->{TrapNumber}==5) {
        $cmd='shutdown -h now';
    } elsif ($trapParams->{TrapNumber}==18||$trapParams->{TrapNumber}==6||$trapParams->{TrapNumber}==10) {
        $cmd='shutdown -c';
    }
} elsif ($trapParams->{Enterprise}=~/USC-IGFAE-MIB::traps/) {
    if ($trapParams->{TrapNumber}==3) {
        $cmd='shutdown -h now';
    } elsif ($trapParams->{TrapNumber}==1) {
        #$cmd='shutdown -h +5min';
        $cmd='shutdown -h now';
    } elsif ($trapParams->{TrapNumber}==2) {
        $cmd='shutdown -c';
    }
}
